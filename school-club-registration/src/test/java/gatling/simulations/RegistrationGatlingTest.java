 ...
 */
public class RegistrationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all registrations").get("/api/registrations").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new registration")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created registration").get("${new_registration_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created registration").delete("${new_registration_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Registration entity").exec(scn);
