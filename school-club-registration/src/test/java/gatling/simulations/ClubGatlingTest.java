 ...
 */
public class ClubGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all clubs").get("/api/clubs").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new club")
                        .post("/api/clubs")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" + "\"name\": \"SAMPLE_TEXT\"" + ", \"description\": \"SAMPLE_TEXT\"" + ", \"maxMembers\": 0" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created club").get("${new_club_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created club").delete("${new_club_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Club entity").exec(scn);
