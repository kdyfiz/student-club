...
/**
 * Performance test for the StudentClubRegistration entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.10.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class StudentClubRegistrationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all studentClubRegistrations")
                    .get("/api/student-club-registrations")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new studentClubRegistration")
                        .post("/api/student-club-registrations")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"registrationDate\": \"2020-01-01T00:00:00.000Z\"" + ", \"status\": \"PENDING\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_studentClubRegistration_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created studentClubRegistration")
                            .get("${new_studentClubRegistration_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created studentClubRegistration")
                        .delete("${new_studentClubRegistration_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StudentClubRegistration entity").exec(scn);
